...
class Attendance:
	def __init__(self, device_ip, major, minor, device_user,
				 device_password):
		...
		while True:
			...
			try:
				response = requests.post(url, data=json.dumps(payload), headers=headers,
										 auth=HTTPDigestAuth(self.device_user,
															 self.device_user_password))
	...
	def get_employee_by_device_id(self, employee_no_string):
		...
		employee = frappe.db.get_value("Employee", {"attendance_device_id": employee_no_string},
	...
	def log_employee_attendance(self, emp_no, record, formatted_timestamp):
		...
		if not self.check_duplicate_checkin(emp_no, formatted_timestamp):
			...
			add_log_based_on_employee_field(
				...
				timestamp=formatted_timestamp, employee_fieldname="name",
				device_id=record['employeeNoString']
			)
		else:
			print(
				f"Duplicate check-in found for {emp_no} at {formatted_timestamp}. Skipping log.")
